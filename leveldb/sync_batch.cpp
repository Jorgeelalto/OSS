...
int insert (leveldb::DB& db, std::atomic<unsigned long>& ctr, int n, char*& block) {
	...
	int i;
	//Set the commit type to synchronous
	leveldb::WriteOptions write_options;
	write_options.sync = true;
	//Create the batch to store insertions inside
	leveldb::WriteBatch batch;
	//While there are still elements to be inserted
	for (i = 0; i < n; i++) {
		//Add data to the batch
		batch.Put(std::to_string(i), block);
		//Once the batch has N operations, insert it into the database
		if ((i % N) == 0) {
			m.lock();
			db.Write(leveldb::WriteOptions(), &batch);
			m.unlock();
		}
		//std::cout << "Stored the block with the key " << i << std::endl;
		//Atomically add one to the petition counter
		ctr.fetch_add(1);
	}
	//If the number of insertions is not a multiple of N then there are elements that have
	//been added to the last batch but it has not been put on the DB, so do it now
	if ((n % N) != 0) db.Write(leveldb::WriteOptions(), &batch);
	return 0;
}
...
int read (leveldb::DB& db, std::atomic<unsigned long>& ctr, int n) {
	...
	for (i = 0; i < n; i++) {
		...
		m.unlock();
		//std::cout << "Read the key " << i << std::endl;
		//Atomically add one to the petition counter
		ctr.fetch_add(1);
	}
	return 0;
